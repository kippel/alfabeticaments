...
async def seed_user(db):
    ...
    try:
        qwerty = bcrypt_context.hash(raw_pw)
    except Exception as exc:
        # If the bcrypt backend is missing or misconfigured in the environment
        # (for example inside a minimal container), don't crash the seeder.
        # Use a SHA256 fallback for development only and warn the user.
        print(
            "Warning: bcrypt not available or misconfigured; using sha256 fallback for dev only:",
            exc,
        )
        qwerty = short_pw

    users = {"username": "admin_run", "password": qwerty}
...
async def run_app():
    ...
    print("run Seeding development application data...")
